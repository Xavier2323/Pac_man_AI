    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                Next = (nextx, nexty)
                if Next in self.corners and Next not in successorVisitedCorners:
                    successorVisitedCorners.append(Next)
                successor = ((Next, successorVisitedCorners), action, 1)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while unvisitedCorners:
        heuristic_cost = sum([util.manhattanDistance(currentPoint, e) for e in unvisitedCorners])
        return heuristic_cost
      
    # totalCost = 0
    # i = len(unvisitedCorners)
    # while unvisitedCorners:
    #     heuristic_cost, selectedCorner = min([(util.manhattanDistance(currentPoint, e), e) for e in unvisitedCorners])
    #     unvisitedCorners.remove(selectedCorner)
    #     totalCost += (heuristic_cost * i)
    #     i = i - 1
    # return totalCost
